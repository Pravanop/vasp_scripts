...
parser.add_argument(
	'-b' , '--band' , metavar = 'bandpath' , nargs = '+' ,
	default = 'band.yaml' ,
	help = 'path to Phonopy (band_).yaml'
	)
parser.add_argument(
	'--bandcolours' , metavar = 'Band colours' , nargs = '+' ,
	default = ['#0026ff' , '#001dc0' , '#4661ff'] ,
	help = 'colours for the band'
	)
parser.add_argument(
	'--bandlabels' , metavar = 'legend labels for bands' , nargs = '+' ,
	default = ['0' , 'C2' , 'C4'] ,
	help = 'legend labels for phonon dispersions for different supercells'
	)
parser.add_argument(
	'--linestyles' , metavar = 'line styles' , nargs = '+' ,
	default = ['-' , '-' , '-'] ,
	help = 'linestyles for phonon dispersions for different supercells'
	)
parser.add_argument(
	'-o' , '--output' , metavar = 'output file suffix' ,
	default = '' ,
	help = 'suffix to add at the end of output file'
	)
parser.add_argument(
	'--style' , metavar = 'style sheet' , nargs = '+' ,
	default = [] ,
...
def add_band(axis , bandfiles , colours , labels , linestyles) :
	...
	if len(bandfiles) > 1 :
		...
		for i in range(len(bandfiles)) :
			axis.plot(
					dists[0] , eigenvalues[i] , linewidth = 2.5 , label = labels[i] , color = colours[i] ,
...
if args.bandlabels is not None :
	...
	for i in range(len(tags)) :
		if tags[i] not in tag :
			...
			tag.append(tags[i])
	legend = ax.legend(
		handle , tag , frameon = False , prop = {'size' : 36} , loc = 'upper left' , bbox_to_anchor = (0.98 , 0.95)
		)
